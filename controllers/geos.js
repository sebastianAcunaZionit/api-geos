...
const fieldExiste = async (field_id) => {
  ...
  try {
    const poligono = await axios.get(
      `https://gate.eos.com/api/cz/backend/api/field/${field_id}?api_key=${process.env.APIKEYGEOS}`
    );
...
const poligonoExiste = async (polygon_id) => {
  ...
  try {
    const poligono = await axios.get(
      `${process.env.URLBASEFEATURE}/${polygon_id}`,
      {
...
const taskStatus = async (task_id, responseType) => {
  ...
  try {
    const task = await axios.get(
      `https://gate.eos.com/api/gdw/api/${task_id}?api_key=${process.env.APIKEYGEOS}`,
  ...
  try {
    ...
    if (!polygon_id) {
      return response.status(496).json({
        ok: false,
        msg: "Debe incluir un id de polygono.",
      });
    }
    ...
    if (!poligono) {
      return response.status(404).json({
        ok: false,
        msg: "poligono no encontrado en EOS.",
      });
    }
    ...
    if (ambiente === "desarrollo") {
      accion = sistema === "export" ? 1 : 2;
    } else if (ambiente === "produccion") {
      accion = sistema === "export" ? 3 : 4;
    } else {
      accion = sistema === "export" ? 5 : 6;
    }

    const bdCon = await conectarseABd(accion);

    const existeAnexo = await bdCon.query(
      `SELECT * FROM anexo_contrato WHERE id_field_eos = ${polygon_id} `
    );

    if (existeAnexo[0].length <= 0) {
      return response.status(404).json({
        ok: false,
        msg: "Polygono no esta asociado a ningun anexo.",
      });
    }
    ...
    const urlstatic = `${process.env.URLBASESTATS}/${stats.data.task_id}?api_key=${process.env.APIKEYGEOS}`;
    ...
    if (errors.length > 0) {
      ...
      const existeDato = await bdCon.query(
        `SELECT * FROM datos_geos WHERE id_poligono_geos = ${polygon_id} `
      );
